...
update f (dependencyGraph, predesossorGraph, cloneGraph) =
    dfv seeds ctl accfn init dependencyGraph
    where
        seeds = Map.keysSet dependencyGraph
        ctl = Map.keysSet cloneGraph
        accfn = update' f
        init = (dependencyGraph, predesossorGraph,cloneGraph)

update' :: Ord k => (Nemo k -> k -> k) -> Nemo k -> k -> Nemo k
update' f (dependencyGraph, predesossorGraph, cloneGraph) original =
    ...
    where
        ...
        clone = f (dependencyGraph, predesossorGraph, cloneGraph) original
        ...
        newPredecessorGraph = Map.insert clone oldClone predesossorGraph
